...
ChemCS supports linear and cosine~\cite{nichol2021improved} noise
...
Figure~\ref{fig:training} shows the training loss over 200 epochs.
...
generator without constraints produces mostly invalid outputs. We
hypothesise that with trained weights the supervisor would address
remaining edge cases, but this has not yet been tested.
...
\emph{Trained-model evaluation.} The benchmark uses only an untrained
GNN. Re-running supervised vs.\ unsupervised generation with trained
weights is the most critical next step to determine whether full
conservation improves from 0\%.
...
QM9~\cite{ramakrishnan2014qm9} or ZINC~\cite{irwin2012zinc}. The
...
message passing (as in DiGress~\cite{vignac2023digress}) and comparing
