...
combustion, acid-base) with 50 random seeds each, the supervisor
enforces 100\% valency validity (every atom respects its bond-count
limit) compared with 12--22\% for unsupervised generation with the same
untrained network. Global conservation constraints (mass, charge, atom
counts) remain unsatisfied at this scale, as they require a trained model
that can predict chemically meaningful element types.
\end{abstract}
...
Hoogeboom et al.~\cite{hoogeboom2022equivariant} introduced an E(3)
...
Vignac et al.~\cite{vignac2023digress} proposed DiGress, a discrete
...
used in DiGress~\cite{vignac2023digress}) could improve the quality of
...
requiring PyTorch at inference time. The NumPy inference path was chosen
to eliminate the PyTorch dependency for deployment; in a production
setting, \texttt{torch.no\_grad()} on CPU would be equally viable.
...
The 0\% full conservation result is equally important. Mass, charge, and
atom-count conservation are global, reaction-level constraints that
require the generated product to contain the correct elements in the
correct quantities. With random GNN weights, the model assigns element
types essentially at random (e.g., predicting nitrogen where carbon
should be), so no amount of local repair can fix the element composition.
Full conservation requires a trained model that produces chemically
meaningful element predictions. The current benchmark does not evaluate
the trained model on the generation task (see Section~\ref{sec:future}),
so the question of whether the supervisor improves full conservation
with a competent model remains open.
...
can enforce local structural validity (specifically, valency limits)
even when the underlying neural network has not been trained. This is a
useful but limited result: the supervisor is effectively performing a
deterministic repair on each atom's bond count, which it can do
regardless of the quality of the model's predictions. The 100\% valency
result should therefore be read as evidence that the
commit/correct/backtrack loop works correctly as an engineering system,
not as evidence that the generated molecules are chemically meaningful.

The 0\% full conservation result underscores this point. Generating
chemically valid reaction products requires the model to predict the
correct element types, which in turn requires training on real molecular
data. The supervisor can repair local bonding errors, but it cannot
invent the correct atom identities. This gap between local and global
validity is the central open question for future work.

Despite these limitations, the results support a broader design
principle: logic and learning are complementary. The neural network
provides generative capacity (proposing structures from noise), while
the constraint system provides domain knowledge (rejecting or repairing
implausible configurations). Neither component alone is sufficient. A
constraint checker without a generator cannot propose novel molecules,
and a generator without constraints produces mostly invalid outputs. The
hypothesis is that with a trained model producing roughly correct
element assignments, the supervisor's repair mechanisms would address
the remaining edge cases, but this hypothesis has not yet been tested.
...
\emph{Missing trained-model evaluation.} The most significant gap in the
current evaluation is that the benchmark (Table~\ref{tab:results})
uses only an untrained GNN with random weights. The training pipeline
(Section~\ref{sec:training}) demonstrates that the loss converges, but
the trained model is never evaluated on the generation benchmark. A
critical next step is to re-run the supervised vs.\ unsupervised
comparison with trained weights to determine whether full conservation
(mass, charge, atoms) improves from 0\% to a meaningful level.
...
(134k molecules) or ZINC~\cite{irwin2012zinc} (millions of compounds),
parsed from SMILES strings via RDKit.

\emph{Model capacity.} The GNN denoiser is intentionally lightweight:
two layers, approximately 20,000 parameters, mean-aggregation without
edge features. This is sufficient for the proof-of-concept evaluation
but unlikely to generalize to larger molecules or more complex reaction
types. Scaling to a deeper model with edge-aware message passing (as
in DiGress~\cite{vignac2023digress}) would be necessary for a
meaningful comparison with the state of the art.
...
reactions, which is sufficient for a proof-of-concept but too small to
draw strong statistical conclusions. Scaling to hundreds of diverse
reaction types would provide more robust evidence.
...
through a curriculum schedule, providing gradient signal that
complements the inference-time checks. The training loss converges on a
small dataset, but the trained model has not yet been evaluated on the
generation benchmark, and this is the most important next step.
